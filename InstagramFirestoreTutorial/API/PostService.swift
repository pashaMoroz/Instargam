...
struct PostService {
    ...
    static func fetchPost(witnPostId postId: String, completion: @escaping(Post) -> Void) {
    ...
    static func likePost(post: Post, comletion: @escaping(FirestoreCompletion)) {
        guard let uid = Auth.auth().currentUser?.uid else { return }
        ...
        COLLECTION_POSTS.document(post.postId).collection("post-likes").document(uid).setData([:]) { _ in
            ...
            COLLECTION_USERS.document(uid).collection("user-likes").document(post.postId).setData([:], completion: comletion)
        }
    }
    
    static func unlikePost(post: Post, comletion: @escaping(FirestoreCompletion)) {
        guard let uid = Auth.auth().currentUser?.uid else { return }
        ...
        COLLECTION_POSTS.document(post.postId).collection("post-likes").document(uid).delete { _ in
            COLLECTION_USERS.document(uid).collection("user-likes").document(post.postId).delete(completion: comletion)
        }
    }
